...
class CAFBA_Model(cobra.Model):
	...
	def uptakeConstraint(self, upt = 1, transporters =[],auto_transporters =True): #I am still not sure about this one... 
		''' set real estate cost on global uptake '''
		UPT_met = cobra.Metabolite('UPT_met',formula='',name='Uptake cost fake metabolite', compartment='c')
		if auto_transporters == True or len(transporters) >0:
			#Automatically identifies transporters by looking at metabolite compartments
			transporters = self.transporters_list()
		for rxn in self.reactions:
			if rxn.id in transporters:
				rxn.add_metabolites({UPT_met : -upt},combine=False)
			else:
				continue
			
		#finishing by adding uptake budget
		uptake_rxn = cobra.Reaction('uptake_rxn')
		uptake_rxn.name = 'Uptake budget reaction'
		uptake_rxn.lower_bound = 0
		uptake_rxn.upper_bound = 34.71 #See uptk_costraint_calculation
		uptake_rxn.add_metabolites({UPT_met : 1})
		self.add_reactions([uptake_rxn])
